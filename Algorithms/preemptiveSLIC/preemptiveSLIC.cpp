    ...
    const double&               M)
{
    ...
    vector<unsigned int> nChangesVec(numk, sz);
...
//         distvec.assign(sz, DBL_MAX); // TODO CHANGED: uncommented
        ...
        for( int r = 0; r < m_height; r++ )
        {
            for( int c = 0; c < m_width; c++ )
            {
              ...
              if(klabels_new[ind]!=klabels[ind])
              {
                  ...
                  x_seed = (c-m_pixel_offset)/m_sx;
                  y_seed = (r-m_pixel_offset)/m_sy;
    ...
    if(klabels_new) delete[] klabels_new;
...
//   GetLABXYSeeds_ForGivenStepSize(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, STEP, perturbseeds, edgemag);
  ...
  double STEP = max(m_sx, m_sy);
//   PerformSuperpixelSLIC(m_kseedsl, m_kseedsa, m_kseedsb, m_kseedsx, m_kseedsy, klabels, STEP, edgemag,compactness);
