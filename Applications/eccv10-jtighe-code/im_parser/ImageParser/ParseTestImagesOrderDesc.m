function timing = ParseTestImagesOrderDesc(HOMEDATA,HOMETESTDATA,HOMELABELSETS,testFileList,testGlobalDesc,trainFileList,trainGlobalDesc,trainIndex,trainCounts,labelPenality,Labels,classifiers,globalSVM,testParams,fullSPDesc)
...
for i = range
    ...
    probPerLabelPerDesc = cell(size(HOMELABELSETS));
    ...
    for labelType=1:length(HOMELABELSETS)
        ...
        if(isempty(classifiers{labelType}))
            ...
            [probPerLabel{labelType} foo  probPerLabelPerDesc{labelType}] = GetAllProbPerLabel(fullfile(DataDir,labelSet),baseFName,probSuffix,retSetIndex,[],labelNums,trainCounts{labelType,Kndx},probType,testParams.smoothingConst,1); %#ok<AGROW>
            ...
            if(isempty(probPerLabel{labelType}))
                ...
                [probPerLabel{labelType} timing(i,3) probPerLabelPerDesc{labelType}] = GetAllProbPerLabel(fullfile(DataDir,labelSet),baseFName,probSuffix,retSetIndex,rawNNs,labelNums,trainCounts{labelType,Kndx},probType,testParams.smoothingConst,1);
            end
    ...
    for numD = 1:length(testParams.segmentDescOrder)
        endStr = [myN2S(numD,2) testParams.segmentDescOrder{numD}];
        for labelType=1:length(HOMELABELSETS)
            if(~isempty(probPerLabelPerDesc{labelType}))
                probPerLabel{labelType} = zeros(size(probPerLabel{labelType}));
                for dndx = 1:numD
                    df = strcmp(testParams.segmentDescOrder{dndx},testParams.segmentDescriptors);
                    probPerLabel{labelType} = probPerLabel{labelType} + probPerLabelPerDesc{labelType}(:,:,df);
                end
            end
            probPerLabel{labelType}(:,~shortListMask{labelType}) = min(probPerLabel{labelType}(:))-1;
            dataCost{labelType} = testParams.maxPenality*(1-1./(1+exp(-(testParams.BConst(2)*probPerLabel{labelType}+testParams.BConst(1)))));
