...
% -------------------------------------
%
% main_runBenchmarkCompactness(alg_paramfilename, alg_paramfilename)
%
...
% computes the compactness error metric based on diameter. Results are 
% stored to disk at the location specified in the alg_paramfilename
%
% Input:
% alg_paramfilename ... algorithm parameter file (YAML)
% bsds_paramfilename ... dataset parameter file (YAML)
%
function main_runBenchmarkCompactnessDiameter(alg_paramfilename, bsds_paramfilename)
      
    fprintf('main_runBenchmarkCompactness, %s and %s\n', alg_paramfilename, bsds_paramfilename);
    ...
    for s=1:numel(params.segParams.set)        
        ...
        % store to disk
        savePath =  fullfile(loadPathBase, ['benchmarkResultCompactnessDiameter.mat']); 
