...
% -------------------------------------
%
% main_evaluateSintelBenchmarkMDE(parameterfiles, ...
...
%
function main_evaluateSintelBenchmarkMDE(parameterfiles, ...
                                         sintel_paramfilename, ...
                                         errorbar_flag, ...
                                         colors, ...
                                         lineStyles, ...
    ...
    if numel(parameterfiles)==1
      flowD_vec = [1];              
      R = loadEvaluationResultsSintel(parameterfiles{1}, sintel_paramfilename, flowD_vec);
    else      
      flowD_vec = [1];
      for i=1:numel(parameterfiles)
        Q = loadEvaluationResultsSintel(parameterfiles{i}, sintel_paramfilename, flowD_vec);
        R(i) = Q(1);
      end  
    end
    ...
    hold on;
    xlabel('# Segments');
    ylabel('Weighted Flow Gradient Error');
    grid on;
    for idx=1:numel(R)
%         h(idx) = plot(     R(idx).mean_nBoundaryPixels_vec, R(idx).mean_weightedFlowGradientError_vec, 'color', C(idx,:), 'LineStyle', L{idx});
        if ~R(idx).failedFlag
          h(idx) = plot( R(idx).mean_nSegments_vec, ...
                         R(idx).mean_weightedFlowGradientErrorRatio_vec, ... % .*  R(idx).mean_nBoundaryPixels_vec , ...
                         'color', colors(idx,:), ...
...
%     export_fig('-r300', 'results/figures/motionGradient_error.pdf');
      
    
end

% load stored results of main_runBenchmarkMDE
function R = loadEvaluationResultsSintel(alg_paramfilename, sintel_paramfilename, flowD_vec)
